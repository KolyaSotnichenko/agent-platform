...
/**
 * Перевіряє, чи доступний агент для поточного користувача
 */
export function isAgentAccessible(
  agent: Agent,
  currentUserId?: string,
): boolean {
  // Системні агенти доступні всім
  if (isSystemCreatedDefaultAssistant(agent)) {
    return false;
  }

  // Якщо немає created_by — вважаємо системним (беккомпат)
  if (!agent.metadata?.created_by) {
    return true;
  }

  // Публічні — всім
  if (agent.metadata?._x_oap_is_public === true) {
    return true;
  }
  ...
  // Приватні — лише власнику
  return agent.metadata?.created_by === currentUserId;
}

/**
 * Перевіряє, чи може користувач отримати доступ до агента
 */
export function canUserAccessAgent(agent: Agent, userId?: string): boolean {
  return isAgentAccessible(agent, userId);
}
